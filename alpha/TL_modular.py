#SAVE CODE IN CASE I MESS UP MAIN SCRIPT 18/09/25
...
def run_top_logic_on_frame(
    ...
    *,
    ...
    # 4b) Decide lateral (run scan ONLY if the geometry gate is active and not in cooldown)
    scan_triggered = False
    scan_reason = None
    sideways_action = None
    scan_dbg = None

    nowm_scan = time.monotonic()
    scan_cooldown_active = nowm_scan < _LTL_SCAN_DISABLED_UNTIL

    if not lat_move_active:
        _p(pfx, "[scan] skipped: LAT_Move gate OFF (dy≥thr)")
    elif scan_cooldown_active:
        _p(pfx, f"[scan] suppressed: lateral-scan cooldown {(_LTL_SCAN_DISABLED_UNTIL - nowm_scan):.2f}s remaining")
    else:
        scan_triggered = True
