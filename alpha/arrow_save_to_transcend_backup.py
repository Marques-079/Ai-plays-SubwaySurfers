...
# Cancel-pair window: if LEFT<->RIGHT happen within this window, delete both frames
CANCEL_PAIR_WINDOW_S = 0.75
...
def _maybe_delete_cancel_pair(window_s=CANCEL_PAIR_WINDOW_S):
    """
    If the last two *directional* saves are (left,right) or (right,left) within window_s,
    delete both files and remove their records from recent/state.
    """
    window_ns = int(window_s * 1e9)

    # Grab last two directional (left/right) records from in-memory recent
    with REC_LOCK:
        dir_recs = [rec for rec in reversed(RECENT_LOCAL)
                    if rec.get("key") in ("left", "right")]
        if len(dir_recs) < 2:
            return
        a = dir_recs[0]  # most recent
        b = dir_recs[1]  # previous
        ...
        # We will remove these two from memory/state and delete files
        targets = (a, b)

        # Remove from in-memory deque first
        for rec in targets:
            try:
    ...
    # Remove the same two from persisted state 'recent'
    state = _read_state() or {}
    lst = state.get("recent", [])
    target_paths = [r.get("path") for r in targets if r.get("path")]
    target_set = set(target_paths)
    state["recent"] = [
        it for it in lst
        if not (isinstance(it, dict) and it.get("path") in target_set)
    ]
    _write_state(state)
    ...
    try:
        print(f"[arrow-saver] Cancel pair ({b['key']} -> {a['key']}) "
              f"in {dt/1e9:.3f}s — deleted {target_paths}")
    except Exception:
        pass

def _maybe_mark_double_tap(window_s=DOUBLE_TAP_WINDOW_S):
    """
    If the last two *directional* saves are (left,left) or (right,right) within window_s,
    rename both files to dl_* or dr_* respectively, and update recent/state.
    """
    window_ns = int(window_s * 1e9)

    def _rename_record_inplace(rec, new_key):
        ...
        i = base.find("_")
        uid = base[i+1:] if i >= 0 else base
        new_path = os.path.join(d, f"{new_key}_{uid}")
        try:
            ...
            lst = state.get("recent", [])
            for it in lst:
                if isinstance(it, dict) and it.get("path") == old_path:
                    it["path"] = new_path
                    it["key"]  = new_key
                    break
            state["recent"] = lst
    ...
    # Look at last two *directional* records that are still raw 'left'/'right'
    with REC_LOCK:
        dir_recs = [rec for rec in reversed(RECENT_LOCAL)
                    if rec.get("key") in ("left", "right")]
        if len(dir_recs) < 2:
        ...
        ka, kb = a.get("key"), b.get("key")
        if ka != kb:   # must be same direction
            return
        if ka not in ("left", "right"):
            return
        ...
        new_key = "dl" if ka == "left" else "dr"

    # Perform renames outside the lock to avoid holding it during IO
    new_a = _rename_record_inplace(a, new_key)
    new_b = _rename_record_inplace(b, new_key)

    try:
        if new_a or new_b:
            print(f"[arrow-saver] Double tap {ka}+{kb} in {dt/1e9:.3f}s — relabeled to {new_key}")
    except Exception:
        pass
...
def _saver():
    while not STOP.is_set():
        ...
        uid = f"{_b36(t_ns)}-{os.getpid()}-{next(_SEQ)}"
        ...
        try:
            ...
            _maybe_mark_double_tap(DOUBLE_TAP_WINDOW_S)
            # NEW: remove canceling pairs (LEFT<->RIGHT) if within window
            _maybe_delete_cancel_pair(CANCEL_PAIR_WINDOW_S)
