...
IN_DIR  = Path("frames")
OUT_DIR = Path("frames_plus2")   # output folder
...
# --------- Probe-line (Jake center) ----------
# Only LEFT and MID rays now
# angle_deg is from vertical (neg = left/CCW, pos = right/CW)
...
RAY_STEP_PX = 2
PROBE_BAND  = 0            # 0 = single-pixel; >0 = ±band around the ray
EXCLUDE_CLASSES = {9}      # ignore RAILS as "obstacle"
# ---------------------------------------------

# BGR colours per class id (with deeper GREYTRAIN)
CLASS_COLOURS = {
...
def shade_and_outline(base_gray_bgr: np.ndarray,
    ...
    for m, c in zip(masks_np[:n], classes_np[:n]):
        ...
        if mask.any():
            ...
            out[mask] = ((1.0 - ALPHA) * region + ALPHA * colvec).astype(np.uint8)
...
def _clampi(v, lo, hi): return lo if v < lo else (hi if v > hi else v)
...
def run_on_image(img_path: Path, save_path: Path):
    ...
    for ray in RAYS:
        ...
        text = f'{ray["name"]}: none' if cls_id is None \
               else f'{ray["name"]}: {LABELS.get(cls_id, cls_id)} @ {dist_px:.0f}px'
